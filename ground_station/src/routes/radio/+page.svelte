<script lang="ts">
    ...
    import {ClearBuffer, DataBits, FlowControl, Parity, StopBits} from "tauri-plugin-serialplugin";
    ...
    let addCR: boolean = $state(false);
  
    const dataBitsOptions: DataBits[] = [DataBits.Five, DataBits.Six, DataBits.Seven, DataBits.Eight];
    ...
    async function sendMessage() {
      try {
        if (serialport && message) {
          ...
          await serialport.write(finalMessage);
    ...
    async function read() {
        if (serialport) {
            ...
            console.error('Serial port is not defined');
        }
    }
  ...
  <main class="container">
    ...
    <div class="section">
      <h2>Connection</h2>
      {#if isConnected}
          <h3>Connected to: {serialport ? serialport.options.path : ''}</h3>
      {:else}
      ...
      <div class="row">
        ...
        <button onclick={disconnect} disabled={!isConnected}>
          {#if isConnected}
              Disconnect from {serialport ? serialport.options.path : ''}
          {:else}
              Disconnect
          {/if}
        </button>
    ...
    <div class="section">
      ...
      <div class="setting">
        <label for="addCR">Add CR:</label>
        <input type="checkbox" id="addCR" bind:checked={addCR} />
      </div>
